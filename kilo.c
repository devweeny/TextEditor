...
void editorUpdateSyntax(erow* row) {
  ...
  char* scs = E.syntax->singleline_commend_start;
...
void editorProcessKeypress() {
  ...
  case DEL_KEY:
    if (c == DEL_KEY) {
      // Prevents cursor from moving to the right if we are on the last char of the last line
      if (E.cy + 1 != E.numrows || E.cx != E.row[E.cy].size) {
